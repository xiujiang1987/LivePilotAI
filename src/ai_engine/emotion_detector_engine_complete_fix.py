# filepath: d:\AI_Park\Workspace\dev_projects\ai\LivePilotAI\src\ai_engine\emotion_detector_engine_complete_fix.py
"""
LivePilotAI - 情感檢測引擎 (完全修復版)
...
# 修復 TensorFlow/Keras 導入問題
try:
    # 測試 TensorFlow keras 可用性
    _ = tf.keras.Model
    logger.info("TensorFlow Keras 可用")
except AttributeError:
    logger.error("無法使用 TensorFlow Keras，請確認 TensorFlow 安裝正確")
    sys.exit(1)
...
class EmotionDetectorEngine(AIEngineBase):
    ...
    async def _init_face_detector(self):
        ...
        try:
            # 修復 OpenCV 級聯檔案路徑問題
            try:
                # 嘗試使用 cv2.data.haarcascades
                cascade_path = cv2.data.haarcascades + cascade_file
            except AttributeError:
                # 如果 cv2.data 不存在，使用預設路徑
                import os
    ...
    async def _detect_faces(self, frame: np.ndarray) -> List[Tuple[int, int, int, int]]:
        ...
        if self.face_cascade is not None:
            ...
            # 修復返回類型問題：確保返回 List[Tuple[int, int, int, int]]
            face_list = []
            for face in faces:
                x, y, w, h = face
